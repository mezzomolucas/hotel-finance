...
export const ExpensesPage: React.FC = () => {
  ...
  const filteredExpenses = useMemo(() => {
    return expenses.filter(expense => {
      ...
      if (sortConfig.key === 'description') return a.description.localeCompare(b.description) * direction;
      if (activeTab === 'payable') {
        return (new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime()) * direction;
      }
      return (new Date(b.paymentDate || b.dueDate).getTime() - new Date(a.paymentDate || a.dueDate).getTime()) * direction;
    });
  ...
  const summary = useMemo(() => {
    ...
    const dueToday = expenses.filter(e =>
      (e.status === 'Pending' || e.status === 'Overdue') && e.dueDate.split('T')[0] === todayStr
    );
    const dueTomorrow = expenses.filter(e =>
      (e.status === 'Pending' || e.status === 'Overdue') && e.dueDate.split('T')[0] === tomorrowStr
    );
    const overdue = expenses.filter(e =>
      e.status === 'Overdue' || (e.status === 'Pending' && e.dueDate.split('T')[0] < todayStr)
    );

    return {
      todayCount: dueToday.length,
      todayTotal: dueToday.reduce((acc, e) => acc + e.amount, 0),
      tomorrowCount: dueTomorrow.length,
      tomorrowTotal: dueTomorrow.reduce((acc, e) => acc + e.amount, 0),
      overdueCount: overdue.length,
      overdueTotal: overdue.reduce((acc, e) => acc + e.amount, 0),
    };
  }, [expenses]);
  ...
  const handleExport = () => {
    ...
    const csvContent = [
      ...
      ...filteredExpenses.map(e => [
        `"${e.description}"`, e.category, e.dueDate, e.paymentDate || '-', e.amount, e.status, e.hasReceipt ? 'Sim' : 'Não',
      ].join(','))
    ].join('\n');
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `expenses_${activeTab}_${localDateStr()}.csv`;
    link.click();
  };
  ...
  const handleOpenModal = (expense?: Expense) => {
    if (expense) {
      ...
      setFormData({
        ...
        paymentDate: activeTab === 'history' ? localDateStr() : '',
    ...
    <div className="space-y-6">
      ...
      <div className="flex space-x-1 bg-white/5 p-1 rounded-lg w-full md:w-fit border border-white/10">
        <button
          ...
          className={`flex-1 md:flex-none px-6 py-2 rounded-md text-sm font-medium transition-all flex items-center justify-center gap-2 ${activeTab === 'payable' ? 'bg-white text-black shadow-sm' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
        >
        ...
        <button
          ...
          className={`flex-1 md:flex-none px-6 py-2 rounded-md text-sm font-medium transition-all flex items-center justify-center gap-2 ${activeTab === 'history' ? 'bg-white text-black shadow-sm' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
        >
        ...
        <div className="w-full md:w-48">
          <CyberSelect
            ...
            options={[
              ...
              ...(activeTab === 'payable'
                ? [{ value: 'Pending', label: 'Pendente' }, { value: 'Overdue', label: 'Atrasado' }]
      ...
      <CyberCard>
        {filteredExpenses.length > 0 ? (
          <Table headers={['Descrição', 'Categoria', 'Vencimento', 'Pagamento', 'Valor', 'Status', 'Recibo', 'Ações']}>
            {filteredExpenses.map(expense => (
              <TableRow key={expense.id}>
                ...
                <TableCell>
                  <span className="px-2 py-1 rounded-md bg-white/5 text-xs text-gray-300 border border-white/10">{expense.category}</span>
                </TableCell>
                ...
                <TableCell>
                  <Badge variant={expense.hasReceipt ? 'info' : 'neutral'}>{expense.hasReceipt ? 'Sim' : 'Não'}</Badge>
                </TableCell>
                <TableCell>
                  <div className="flex space-x-2">
                    <button onClick={() => handleOpenModal(expense)} className="text-gray-400 hover:text-white transition-colors"><Edit2 size={16} /></button>
                    <button onClick={() => deleteExpense(expense.id)} className="text-red-500 hover:text-red-400 transition-colors"><Trash2 size={16} /></button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-gray-400 text-sm max-w-sm mx-auto mb-6">Não encontramos nenhuma despesa com os filtros selecionados.</p>
            <CyberButton onClick={() => handleOpenModal()} variant="primary">
              <Plus className="w-4 h-4 mr-2" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-[#141414] border border-white/10 rounded-xl shadow-2xl w-full max-w-md p-6 relative my-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <CyberInput label="Descrição" required value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} />
              <CyberSelect
                label="Categoria"
                value={formData.category}
                onChange={e => setFormData({ ...formData, category: e.target.value as ExpenseCategory })}
                options={[
                  { value: 'Cleaning', label: 'Limpeza' }, { value: 'Utilities', label: 'Utilidades' },
                  { value: 'Staff', label: 'Equipe' }, { value: 'Maintenance', label: 'Manutenção' },
                  { value: 'Supplies', label: 'Suprimentos' }, { value: 'Marketing', label: 'Marketing' },
                  { value: 'Water', label: 'Água' }, { value: 'Electricity', label: 'Luz' },
                  { value: 'Laundry', label: 'Lavanderia' }, { value: 'OTA Commission', label: 'Comissão OTA' },
                  { value: 'Payroll', label: 'Folha de Pagamento' }, { value: 'Other', label: 'Outros' },
                ]}
              />
              <div className="grid grid-cols-2 gap-4">
                <CyberInput label="Valor (R$)" type="number" step="0.01" required value={formData.amount} onChange={e => setFormData({ ...formData, amount: parseFloat(e.target.value) })} />
                <CyberInput label="Vencimento" type="date" required value={formData.dueDate} onChange={e => setFormData({ ...formData, dueDate: e.target.value })} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <CyberSelect
                  label="Status"
                  ...
                  options={[
                    ...
                    { value: 'Overdue', label: 'Atrasado' }, { value: 'Cancelled', label: 'Cancelado' },
                  ]}
                />
                <CyberInput
              ...
              <div className="flex justify-end space-x-3 pt-6">
                <CyberButton type="button" variant="secondary" onClick={() => setIsModalOpen(false)}>Cancelar</CyberButton>
