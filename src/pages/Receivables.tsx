...
import { Plus, Trash2, Edit2, X, Save, Search, Download, FileText, Calendar, AlertCircle, Clock } from 'lucide-react';
...
export const ReceivablesPage: React.FC = () => {
  ...
  const summary = useMemo(() => {
    ...
    const dueToday = receivables.filter(r =>
      (r.status === 'Pending' || r.status === 'Overdue') && r.dueDate.split('T')[0] === todayStr
    );
    const dueTomorrow = receivables.filter(r =>
      (r.status === 'Pending' || r.status === 'Overdue') && r.dueDate.split('T')[0] === tomorrowStr
    );
    const overdue = receivables.filter(r =>
      r.status === 'Overdue' || (r.status === 'Pending' && r.dueDate.split('T')[0] < todayStr)
    );
    const totalPending = receivables.filter(r => r.status === 'Pending' || r.status === 'Overdue');

    return {
      todayCount: dueToday.length,
      todayTotal: dueToday.reduce((acc, r) => acc + r.amount, 0),
      tomorrowCount: dueTomorrow.length,
      tomorrowTotal: dueTomorrow.reduce((acc, r) => acc + r.amount, 0),
      overdueCount: overdue.length,
      overdueTotal: overdue.reduce((acc, r) => acc + r.amount, 0),
      totalPendingAmount: totalPending.reduce((acc, r) => acc + r.amount, 0),
    };
  }, [receivables]);
  ...
  const handleExport = () => {
    ...
    const csvContent = [
      headers.join(','),
      ...filteredReceivables.map(r => [`"${r.source}"`, r.dueDate, r.amount, r.status].join(','))
    ].join('\n');
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `receivables_${localDateStr()}.csv`;
    link.click();
  };
  ...
  const handleOpenModal = (receivable?: Receivable) => {
    if (receivable) {
      ...
      setFormData({ ...receivable, dueDate: receivable.dueDate.split('T')[0] });
    } else {
      setEditingId(null);
      setFormData({ source: 'Booking.com', amount: 0, dueDate: localDateStr(), status: 'Pending' });
    }
    setIsModalOpen(true);
  };
      ...
      if (editingId) {
        ...
        if (original && original.status !== 'Paid' && data.status === 'Paid') {
          await addIncome({
            ...
            checkIn: localDateStr(),
            checkOut: localDateStr(),
            dailyRate: 0,
            extraConsumption: 0,
            total: data.amount,
      ...
      <div className="flex flex-col md:flex-row gap-4 bg-white/5 p-4 rounded-xl border border-white/10">
        ...
        <div className="w-full md:w-48">
          <CyberSelect
            ...
            options={[
              { value: 'All', label: 'Todos os Status' }, { value: 'Paid', label: 'Pago' },
        ...
        <div className="w-full md:w-48">
          <CyberSelect
            ...
            options={[
              { value: 'date-asc', label: 'Mais Antigos' }, { value: 'date-desc', label: 'Mais Recentes' },
              { value: 'amount-desc', label: 'Maior Valor' }, { value: 'amount-asc', label: 'Menor Valor' },
            ]}
          />
        </div>
      </div>
      ...
      <CyberCard>
        {filteredReceivables.length > 0 ? (
              ...
              <TableRow key={receivable.id}>
                ...
                <TableCell>
                  <div className="flex space-x-2">
                    <button onClick={() => handleOpenModal(receivable)} className="text-gray-400 hover:text-white transition-colors"><Edit2 size={16} /></button>
                    <button onClick={() => deleteReceivable(receivable.id)} className="text-red-500 hover:text-red-400 transition-colors"><Trash2 size={16} /></button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-gray-400 text-sm max-w-sm mx-auto mb-6">Não encontramos nenhum recebível com os filtros selecionados.</p>
            <CyberButton onClick={() => handleOpenModal()} variant="primary">
              <Plus className="w-4 h-4 mr-2" />
          ...
          <div className="bg-[#141414] border border-white/10 rounded-xl shadow-2xl w-full max-w-md p-6 relative my-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="block text-xs font-sans font-medium text-gray-400 mb-2 uppercase tracking-wide">Origem / Hóspede</label>
                <input
                  type="text"
                ...
                <datalist id="source-suggestions">
                  <option value="Booking.com" /><option value="Expedia" /><option value="Airbnb" />
                  <option value="Direct" /><option value="Corporate" />
                </datalist>
              </div>
